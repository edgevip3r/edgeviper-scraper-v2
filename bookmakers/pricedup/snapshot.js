...
export default async function snapshotPricedUp({ book, config, bookCfg, outRoot, debug }) {
  ...
  try {
    ...
    // --- Cookiebot consent (based on your popup HTML) ---
    // Try "Allow all" first, then "Accept", then Decline as last resort.
    ...
    // --- Wait for any boosts to render (titles vary, so use generic tokens) ---
    await page.waitForTimeout(800);
    await page.waitForSelector('li[class*="SelectionsGroupLiItem"], [class*="SelectionsGroupName"]', { timeout: 15000 });

    // --- Expand collapsed groups (MarketGroupWrapper / MarketHeaderContent) ---
    // Click headers that are collapsed or that control a hidden list container.
    ...
    // --- Scroll to bottom (hydrate lazy items) ---
    let lastHeight = 0;
    for (let i = 0; i < 10; i++) {
    ...
    if (debug) console.log(`[snapshot:pricedup] cards detected: ${cardsCount}`);
