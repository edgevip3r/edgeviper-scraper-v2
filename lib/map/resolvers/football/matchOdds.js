// lib/map/resolvers/football/matchOdds.js
// Resolver for ALL_TO_WIN → MATCH_ODDS
...
export async function resolveMO(leg, { debug=false, horizonHours=72 } = {}){
  ...
  let cats=[];
  for(let i=0;i<aliasNames.length;i++){
    const q=aliasNames[i]; const r=await fetchCandidatesForTeam(q, MO_CODE, { horizonHours });
  ...
  const withTeam=[];
  for(let i=0;i<cats.length;i++){
    const c=cats[i]; const nonDraw=splitNonDrawRunners(c); let ok=false;
  ...
  for(let i=0;i<withTeam.length;i++){
    ...
    if(debug && drop) console.log('[map:MO] drop self youth/B/women', { eventName:c&&c.event&&c.event.name });
