...
export async function mapMatchOdds(leg, { debug = false, horizonHours = 72 } = {}) {
  ...
  const teamVariants = variantsForCanonical(teamCanon);
  const withTeam = cats.filter(c => {
    ...
    const hasTeam = nonDraw.some(r => {
      const rName = canon(r.runnerName);
      // exact match against any variant or tolerant contains against canonical
      return teamVariants.has(rName) || wordishContains(r.runnerName, teamCanon);
    });
    return hasTeam;
  });
  ...
  for (const c of candidates) {
    const runners = c?.runners || [];
    const nonDraw = runners.filter(r => r?.runnerName && canon(r.runnerName) !== 'the draw');
    const hit = pickTeamRunner(nonDraw, teamCanon);
    if (hit) {
      chosen = c;
      runner = hit;
      break;
