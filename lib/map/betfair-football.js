...
// - Robust team-runner pick (never 'The Draw')
// - Earliest KO among valid candidates
//
// Exports:
...
function pickTeamRunner(runners, targetTeam) {
  const tCanon = canon(targetTeam);
  // 1) exact (case-insensitive) match
  let r = runners.find(x => canon(x.runnerName) === tCanon);
  if (r) return r;
  // 2) tolerant contains (wordish)
  r = runners.find(x => wordishContains(x.runnerName, targetTeam));
  if (r) return r;
  // 3) last resort: prefer any non-draw that is closest by Levenshtein? (skip; keep it deterministic)
  return null;
}
...
export async function mapMatchOdds(leg, { debug = false, horizonHours = 72 } = {}) {
  ...
  const triedName = rawTeam;
  const cats = await fetchMoCandidatesForTeam(rawTeam, { horizonHours });

  if (debug) console.log(`[map:MO] candidates for "${rawTeam}": ${cats.length}`);
  ...
  const withTeam = cats.filter(c => {
    ...
    const hasTeam = runners.some(r =>
      r?.runnerName && r.runnerName.toLowerCase() !== 'the draw' && (
        canon(r.runnerName) === canon(rawTeam) || wordishContains(r.runnerName, rawTeam)
      )
    );
    return hasTeam;
  });
  ...
  const candidates = withTeam.filter(c => {
    const competitionName = c?.competition?.name || '';
    const eventName       = c?.event?.name || '';
    const runnerNames     = (c?.runners || []).map(r => r.runnerName || '');
    const drop = shouldDropCandidate({ competitionName, eventName, runnerNames, targetTeam: rawTeam });
  ...
  // 3) Pick earliest KO among remaining
  candidates.sort(sortByOpenDateAsc);
  let chosen = null;
  let runner = null;
  ...
  for (const c of candidates) {
    const runners = c?.runners || [];
    // NEVER pick the Draw
    const nonDraw = runners.filter(r => r?.runnerName && r.runnerName.toLowerCase() !== 'the draw');
    const hit = pickTeamRunner(nonDraw, rawTeam);
    if (hit) { chosen = c; runner = hit; break; }
  }
  ...
  if (!chosen || !runner) {
    return {
      ...
      triedName,
      candidatesHint: candidates.slice(0, 8).map(c => c?.event?.name).filter(Boolean)
    };
  }
  ...
  if (debug) {
    console.log(`[map:MO] OK "${rawTeam}" -> event="${res.eventName}" | marketId=${res.marketId} | sel=${res.selectionId}`);
  }
  return res;
}
